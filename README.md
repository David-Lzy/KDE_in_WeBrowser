...
- **NAT-friendly exposure options**: private/NAT hosts can use frpc,
  Cloudflare named tunnel, or Cloudflare quick tunnel without exposing raw
  Webtop ports.
...
- **公网只暴露 HTTPS 网关**：Webtop 原始端口留在 Docker 网络里，对外只暴露
  认证后的 HTTPS 入口。
- **适配内网/NAT**：可选择 frpc、Cloudflare named tunnel 或 Cloudflare quick
  tunnel，只暴露认证网关，不暴露 Webtop 原始端口。
...
Browser
  |
  | HTTPS, frpc, or Cloudflare Tunnel
  v
...
recommended value. For sensitive required values such as frpc token,
Cloudflare API token, or Authelia bootstrap password, enter a value or type
`skip` explicitly.

It also detects the host network shape. Private/NAT hosts are guided toward
frpc or Cloudflare Tunnel. Public-direct hosts can opt into an `sslip.io` or
manual domain with Let's Encrypt HTTP-01 certificates and automatic renewal.
...
host PAM helper. Secrets such as frpc and Cloudflare tokens are intentionally
skipped unless you provide them interactively or edit local ignored files.
...
- frpc config path
- Cloudflare Tunnel API, hostname, and tunnel token settings
...
- [frpc](Doc/frpc.md)
- [Cloudflare Tunnel](Doc/cloudflare-tunnel.md)
...
documentation only. Runtime state, host credentials, frpc and Cloudflare
secrets, WeChat/QQ data, TLS private keys, and local workflow notes are
