...
The reusable NGINX template also contains an internal HTTP listener on `8080`
for Docker-network origins such as Cloudflare Tunnel. The default Compose file
publishes only the HTTPS listener. Do not publish `8080` to the host. Public
ACME mode uses Certbot standalone to open TCP port `80` temporarily outside
Docker during certificate issuance and renewal.
